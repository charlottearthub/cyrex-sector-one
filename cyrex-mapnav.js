(function(){
  ...
  let mapMode='sector';
  const districtZones=[
    {id:'grayline',label:'Grayline',x:'45%',y:'31%',icon:'⌖'},
    {id:'dock12',label:'Dock 12',x:'64%',y:'67%',icon:'⇣'},
    {id:'blackrow',label:'Market Row',x:'25%',y:'71%',icon:'◆'},
    {id:'clinic',label:'Clinic',x:'32%',y:'49%',icon:'✚'}
  ];
  const districtActions={
    grayline:[{label:'Scavenge',action:'scavenge-run',x:'35%',y:'58%',icon:'◇'},{label:'Scan',action:'scan-district',x:'58%',y:'37%',icon:'◎'},{label:'Clinic',action:'visit-clinic',x:'67%',y:'70%',icon:'✚'},{label:'Jobs',tab:'jobs',x:'29%',y:'28%',icon:'!'}],
  ...
  function hotspot(h){
    const cls='cyrex-map-hotspot'+(h.warn?' warn':'')+(h.id&&h.id===activeId()?' active':'');
    return `<button class="${cls}" aria-label="${h.label}" title="${h.label}" style="--x:${h.x};--y:${h.y}" ${h.id?`data-map-district="${h.id}"`:''} ${h.action?`data-map-action="${h.action}"`:''} ${h.tab?`data-map-tab="${h.tab}"`:''}><span class="cyrex-map-symbol">${h.icon||'•'}</span><strong>${h.label}</strong></button>`;
  }
